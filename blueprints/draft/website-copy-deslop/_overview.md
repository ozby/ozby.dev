...
tags: []
approvals: [] # 1 Claude/Codex or 2 OpenCode-Go approvals required before draft→planned (see ## Approvals)
---

# Website-copy-deslop

**Goal:** Remove generic marketing filler and repetition from public website copy while preserving claims and behavior.

## Planning Summary

- Goal input: `website-copy-deslop`
- Complexity: `S`
- Draft slug: `website-copy-deslop`
- Output path: `blueprints/draft/website-copy-deslop/_overview.md`
- Generated command: `wp blueprint new "website-copy-deslop" --complexity S`
...
## Approvals (required before promotion to `planned`)

Promotion `draft → planned` requires **either 1 provenance-backed Claude/Codex approval
or 1 provenance-backed OpenCode-Go model approval**, recorded in the
frontmatter `approvals:` list (the gate input; this checklist is a human-readable
mirror). Each counted entry is a real independent review pass tied to a committed
`reviews.md` structured record with a separate tracked transcript/artifact (for
example `artifact: review-artifacts/deepseek-final.md`). Enforced by `wp audit
blueprint-lifecycle` and the `wp blueprint promote` command. See
`catalog/agent/rules/pre-implementation.md`.

- [ ] Claude (`/claude`) or Codex (`/codex`) approval (satisfies the gate alone)
- [ ] OpenCode-Go model approval 1 — `/deepseek` / `/glm` / `/hy3` / `/kimi` /
      `/mimo` / `/minimax` / `/qwen`
- [ ] OpenCode-Go model approval 2 — distinct from approval 1 if no Claude/Codex approval
